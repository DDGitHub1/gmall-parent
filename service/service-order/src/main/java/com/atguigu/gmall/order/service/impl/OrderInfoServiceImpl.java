...
public class OrderInfoServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo>
    ...
    public void changeOrderStatus(Long orderId, Long userId, ProcessStatus closed, List<ProcessStatus> expected) {
        ...
        List<String> collect = expected.stream().map(status -> status.name()).collect(Collectors.toList());
        //幂等修改订单
        orderInfoMapper.updateOrderStatus(orderId,userId,processStatus,orderStatus,collect);
    }
